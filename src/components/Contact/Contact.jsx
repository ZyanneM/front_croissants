...
const Contact = () => {
    ...
    async function fetchComponents() {
        try {
            const response = await fetch(`${strapiEndpoint}/api/contact-page?populate[0]=image&populate[1]=title.image&populate[2]=title.imagetext&populate[3]=calltoaction.button&populate[4]=socialmedias.socialmedia.image`);
            ...
            { title ? (
                  ...
                  <div className="contact title">
                    <div className='contact-blocks'>
                        <div className='cta-container'>
                            <img src={`${strapiEndpoint}${title.imagetext.data.attributes.url}`} className="transformed-title title-project-contact" alt="vous avez un projet ?"/>
                            <img className="icon-title  icon-rocket" src={`${strapiEndpoint}${title.image.data.attributes.url}`} alt="rocket" />
                                <Link to="/formcontact">
                ...
                <div className='contact-infos-container'>
                    <div className="contact-us title">
                        ...
                        <img className="icon-title coffee" src={`${strapiEndpoint}${image.data.attributes.url}`} alt="cafe croissant" />
                    </div>
                        ...
                        <div className={`socialmedias ${socialMedias[0].style}`}>
                            { socialMedias[0].socialmedia.map(socialmedia => (
                                <a href={`${socialmedia.link}`} className="social-media-link" target='_blank'>
                                    <img src={`${strapiEndpoint}${socialmedia.image.data.attributes.url}`} alt="" />
                                </a>
